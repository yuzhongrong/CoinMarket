...
public class KeystoreManagerDebug {
    ...
    // 获取RSA公钥
    private static PublicKey getRSAPublicKey() throws KeyStoreException, CertificateException, NoSuchAlgorithmException, UnrecoverableKeyException {
    ...
    // 获取RSA私钥
    private static PrivateKey getRSAPrivateKey() throws KeyStoreException, CertificateException, NoSuchAlgorithmException, UnrecoverableKeyException {
        KeyStore keyStore = KeyStore.getInstance(ANDROID_KEYSTORE);
        try {
            keyStore.load(null);
        } catch (IOException e) {
